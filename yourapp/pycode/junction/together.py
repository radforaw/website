...
def main(junc):
	...
	print len(n)
	w=450
	til = Image.new("RGB",(w*3,w*int((len(n)+1)/3)),"Black")
	for m in range(len(n)):
		...
		for b in dirs[n[m]]:
			image.onscreen(b[0],b[1],colour=(255,0,0),width=5)
		til.paste(image.image,(w*(m%3),w*int(m/3)))
		font=ImageFont.truetype("montserrat.regular.ttf",24)
		ImageDraw.Draw(til).text(((w*(m%3))+20, (w*int(m/3))+20),chr(64+int(n[m])),(255,255,255),font=font)
